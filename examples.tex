...
% ----------------------------------------------------------------------
\begin{frame}<1-5>[label=operatorframe]{Application operator}
  \begin{itemize}
  ...
  \medskip
  \item<2-> Let $P$ be a set of \alert<2-5>{\only<1-3>{normal}\only<4>{choice}\only<5->{constraint}} 
            rules and $\setsets{X}$ a set of sets of atoms,\par     
            the \alert<2-5>{application operator} $\App{P}$ is defined as follows:
  \smallskip 
  \mbox{\small$
      \A{P}{\setsets{X}} = \big\{  
        \only<1-4>{X \cup Y}\only<5->{\alert<5>{\hspace{9.5pt}X \hspace{10pt}}}\mid X \in \setsets{X}, %
        \only<1-4>{Y}\only<5->{\ \alert<5>{\emptyset}} \only<1-3,5->{\alert<2-3,5>{=}}\only<4>{\alert<4>{\subseteq}} %
        \alt<3>{
        \underbrace{
        \{ \only<1-4>{\head{r}}\only<5->{\hspace{14.5pt} \alert<5>{r} \hspace{14pt}} \mid r\in P\text{ and }%
...
\uncover<7->{
   \item For $n \geq 1$, let \alert<7>{$\Ai{P}{n}{\setsets{X}}$} be $\A{P}{\setsets{X}}$ 
         if $n=1$ and $\A{P}{\Ai{P}{n-1}{\setsets{X}}}$ if $n > 1$.\par
         E.g., $\Ai{P}{2}{\setsets{X}}$ is  $\A{P}{\A{P}{\setsets{X}}}$.
}\uncover<8->{
   \pause
   \item Let \alert<8>{$\Afixp{P}{\setsets{X}}$} be $\Ai{P}{n}{\setsets{X}}$ where $n$ is the smallest integer such that 
