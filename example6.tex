...
\left\{
    \{ a \} \leftarrow, \ \
    \alert<1,8>{\alt<8>{\phantom{\{}b\phantom{\}}}{\{b\}} \leftarrow c}, \ \
    \alert<1>{\{c\} \leftarrow a}, \ \
    %\{a\} \leftarrow \alt<17->{\alert{\naf{q}}}{q}
    \alert<1>{\{a\} \leftarrow b}
\right\}
$
...
\node[smodel, below right=of node12, small, xshift=5mm] (node24) {$a$ $c$ $b$} edge [arrow] (node12);
}}
...
\node[rule, right=of node12, xshift=3cm] (rule_seq_1) {\alt<7>{\scode \{a\}.\hspace{9mm}}{$\{a\} \leftarrow$}};

\alert<1>{
\alert<8>{
\node[rule, below=of rule_seq_1] (rule_seq_2) {\alt<7>{\scode \{b\} :- c.}{$\alt<8>{\phantom{\{}b\phantom{\}}}{\{b\}} \leftarrow c$}};
}
\node[rule, below=of rule_seq_2, yshift=7mm] (rule_seq_3) {\alt<7>{\scode \{c\} :- a.}{$\{c\} \leftarrow a$}};
\node[rule, below=of rule_seq_3, yshift=7mm] (rule_seq_4) {\alt<7>{\scode \{a\} :- b.}{$\{a\} \leftarrow b$}};
}
...
\draw[very thick, dotted, rounded corners] 
  ($ (rule_seq_2.north west) + (1mm,1mm) $) rectangle ($ (rule_seq_4.south east) + (-0.5mm,-1mm) $);
}}
...
\node[align=center] at (11,1.9) {\Large{\textbf{\alert{Later\ldots}}}};
\end{tikzpicture}
}
\end{frame}
